      ...
      <div className='flex flex-col sm:flex-row items-start gap-5 mt-5'>
        ...
        <div className='w-full grid grid-cols-auto gap-4 gap-y-6'>
          {filterDoc.map((item, index) => (
            <div onClick={() => { navigate(`/appointment/${item._id}`); scrollTo(0, 0) }} className='border border-[#C9D8FF] rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] hover:shadow-xl hover:shadow-primary/10 transition-all duration-500 group relative bg-white' key={index}>
              <div className="relative overflow-hidden w-full h-auto aspect-square bg-[#EAEFFF] group-hover:bg-primary transition-colors duration-500">
                  <img className='w-full h-full object-cover object-top group-hover:scale-105 transition-transform duration-500 mix-blend-multiply' src={item.image} alt="" />
              </div>
              <div className='p-4'>
